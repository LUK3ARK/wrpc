...
fn fields_to_wrpc(fields: Vec<(String, Vec<Bytes>)>) -> wrpc_transport_legacy::Value {
    fields
        .into_iter()
        .map(|(k, v)| {
            (
...
#[cfg(feature = "wasmtime-wasi-http")]
#[instrument(level = "trace", skip_all)]
pub fn try_hyper_to_outgoing_request(
    ...
    Ok((
        Request {
            body,
            trailers,
            method: Method::try_from(&method).context("failed to convert method")?,
            path_with_query: uri.path_and_query().map(ToString::to_string),
            scheme: uri.scheme().map(Into::into),
            authority: uri.authority().map(ToString::to_string),
            headers,
...
pub trait IncomingHandler: wrpc_transport_legacy::Client {
    #[instrument(level = "trace", skip_all)]
...
impl<T: wrpc_transport_legacy::Client> IncomingHandler for T {}

pub trait OutgoingHandler: wrpc_transport_legacy::Client {
    #[instrument(level = "trace", skip_all)]
    fn invoke_handle(
        &self,
        request: Request<
    ...
    > + Send {
        use wasmtime_wasi_http::body::HyperOutgoingBody;
        async {
            let (req, opts, errors) = try_hyper_to_outgoing_request(request, options)?;
...
impl<T: wrpc_transport_legacy::Client> OutgoingHandler for T {}
