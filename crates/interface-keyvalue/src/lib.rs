...
use wrpc_transport_legacy::{Acceptor, IncomingInputStream, Value};

pub trait Eventual: wrpc_transport_legacy::Client {
...
impl<T: wrpc_transport_legacy::Client> Eventual for T {}

pub trait Atomic: wrpc_transport_legacy::Client {
    #[instrument(level = "trace", skip_all)]
...
impl<T: wrpc_transport_legacy::Client> Atomic for T {}

/// `wrpc:keyvalue/atomic` invocation streams
pub struct AtomicInvocations<T>
where
    T: wrpc_transport_legacy::Client,
{
